...
function bubbleSort(str) {
    ...
    //把排序最终状态的数组和取消定位指针保存到sortLog中，因为动画的最后状态时不显示比较数据的
...
function showLog(logValues,showElement){
    let str = ''
    //解构赋值的方式，从排序日志遍历器去取一条结果，赋值给row和pos变量
    let{
        done,
        value:[row,pos]
    } = logValues.next()
    //对比一下row和pos的值是否与排序时保存一致
    //console.log(row)
    //console.log(pos)
    //遍历row数组，构建显示当前排序状态的html代码
    for(const key in row) {
        //根据定位指针数据吧进行比较的数据编程红色
