...
var collegeSelect = document.getElementById('collegeSelect');//获取学院对象
createOption(collegeSelect,collegeArr);
var majorSelect = document.getElementById('majorSelect');//获取学科对象
collegeSelect.onchange = function( ) {         //为学院下拉列表添加事件
    majorSelect.options.length=0;             //清空major下所有原有的option
    createOption(majorSelect, majorArr[collegeSelect.value]);
};
var classSelect = document.getElementById('classSelect');//获取班级对象
...
collegeSelect.onchange = function(){
    ...
    if(collegeSelect.value >= 0){
        majorSelect.onchange();    //自动添加 学科对应班级 下拉菜单
    }else {
        classSelect.options.length = 0;       //清空class下原有的option
    }
};
